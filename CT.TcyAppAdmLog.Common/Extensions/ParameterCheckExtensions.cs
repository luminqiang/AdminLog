...
namespace CT.TcyAppAdmLog.Common.Extensions
{
    /// <summary>
    /// 用于参数检查的扩展方法
    /// </summary>
    public static class ParameterCheckExtensions
    {
        ...
        {
            Require<ArgumentNullException>(value != null, string.Format(ParameterCheckConstant.ParameterCheck_NotNull, paramName));
        }
        ...
        public static void CheckNotNullOrEmpty(this string value, string paramName)
        {
            ...
            Require<ArgumentException>(value.Length > 0, string.Format(ParameterCheckConstant.ParameterCheck_NotNullOrEmpty_String, paramName));
        }
        ...
        {
            Require<ArgumentException>(value != Guid.Empty, string.Format(ParameterCheckConstant.ParameterCheck_NotEmpty_Guid, paramName));
        }
        ...
        public static void CheckNotNullOrEmpty<T>(this IEnumerable<T> collection, string paramName)
        {
            ...
            Require<ArgumentException>(collection.Any(), string.Format(ParameterCheckConstant.ParameterCheck_NotNullOrEmpty_Collection, paramName));
        }
        ...
        public static void CheckLessThan<T>(this T value, string paramName, T target, bool canEqual = false) where T : IComparable<T>
        {
            ...
            string format = canEqual ? ParameterCheckConstant.ParameterCheck_NotLessThanOrEqual : ParameterCheckConstant.ParameterCheck_NotLessThan;
        ...
        {
            bool flag = canEqual ? value.CompareTo(target) >= 0 : value.CompareTo(target) > 0;
            string format = canEqual ? ParameterCheckConstant.ParameterCheck_NotGreaterThanOrEqual : ParameterCheckConstant.ParameterCheck_NotGreaterThan;
            Require<ArgumentOutOfRangeException>(flag, string.Format(format, paramName, target));
        }
            ...
            where T : IComparable<T>
        {
            ...
            string message = startEqual
                ? string.Format(ParameterCheckConstant.ParameterCheck_Between, paramName, start, end)
                : string.Format(ParameterCheckConstant.ParameterCheck_BetweenNotEqual, paramName, start, end, start);
            Require<ArgumentOutOfRangeException>(flag, message);

            flag = endEqual ? value.CompareTo(end) <= 0 : value.CompareTo(end) < 0;
            message = endEqual
                ? string.Format(ParameterCheckConstant.ParameterCheck_Between, paramName, start, end)
                : string.Format(ParameterCheckConstant.ParameterCheck_BetweenNotEqual, paramName, start, end, end);
            Require<ArgumentOutOfRangeException>(flag, message);
        }
        ...
        public static void CheckDirectoryExists(this string directory, string paramName = null)
        {
            ...
            Require<DirectoryNotFoundException>(Directory.Exists(directory), string.Format(ParameterCheckConstant.ParameterCheck_DirectoryNotExists, directory));
        }
        ...
        public static void CheckFileExists(this string filename, string paramName = null)
        {
            ...
            Require<FileNotFoundException>(File.Exists(filename), string.Format(ParameterCheckConstant.ParameterCheck_FileNotExists, filename));
        }
    }
}